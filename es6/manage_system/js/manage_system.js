...
let vue=new Vue({
    ...
    data:{
        ...
        saveDetail:[],
        options:[
            {value:'0',text:'按时间的升序排序'},
            {value:'1',text:'按时间的降序排序'}
        ],
        detailItem:
            {
                'task_title':'',
    ...
    computed:{
        ...
        specifyDetail:function(){
           let search=this.searchInfo;
           let showSearch='';
           this.detail = JSON.parse(JSON.stringify(this.detail_copy));
           if(search){
               return this.detail.filter((value)=> {//value是一个对象
                   return Object.keys(value).some((key)=>{
                       let index=String(value[key]).toLowerCase().indexOf(search);
                       if(index>-1){
                          showSearch=this.detail[index];
                          return showSearch;
                       }
    ...
    methods:{
        ...
        search:function(){
            ...
            if(this.searchInfo){
                if(!isNaN(parseInt(this.searchInfo))){//是数字
                    ...
                    for(let i in this.detail){//非数字
                        if(this.detail[i].task_title.indexOf(this.searchInfo)>=0){
                            //console.log(this.detail[i].task_title);
                            this.isTitle=true;
                        }
                        if(this.detail[i].executor.indexOf(this.searchInfo)>=0){
                            this.isExecutor=true;
                        }
                        if(this.detail[i].sender.indexOf(this.searchInfo)>=0){
                            this.isSender=true;
                        }
                    }
                }
        ...
        ascendingSortByTime:function () {
            this.sortByTime(1);
            this.isShowChoose= false;
        },
        descendingSortByTime:function () {
            this.sortByTime(-1);
            this.isShowChoose= false;
        },
        sortByTime:function (flag) {
           let saveTime=[];
            let date = new Date();
            for(let i in this.detail){
                let time=this.detail[i].create_time.split('-');//取出时间
                saveTime.push(date.setFullYear(time[0],time[1] - 1,time[2]));//时间转换成秒
            }
            saveTime.forEach((value,index)=>{
                this.detail[index].create_time=value;
            });
            //时间排序
       ...
       /* ascendingSortByTitle:function () {
        ...
        editConfirm:function () {
            this.checkFlag=false;
            let index=this.detail.indexOf(this.curAddress);
            //console.log(this.curMsg);
        ...
        confirm:function(){
            let index=this.detail.indexOf(this.deleteMsg);
            //console.log(index);
